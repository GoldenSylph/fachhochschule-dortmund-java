...
└────────────────────┬────────────────────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────────────────────┐
│              CoreConfiguration (Singleton Enum)             │
│          Central Coordinator & Dependency Injector          │
├─────────────────────────────────────────────────────────────┤
│  Autowires:                                                 │
│  • AGVManagementConfiguration                               │
...
└────────┬───────────┬────────────┬────────────┬──────────────┘
         │           │            │            │
    ┌────▼────┐ ┌───▼────┐  ┌────▼────┐  ┌───▼─────┐
    │Clocking │ │  Task  │  │ Storage │  │Observer │
    │ System  │ │  Mgmt  │  │  Mgmt   │  │ System  │
    └────┬────┘ └───┬────┘  └────┬────┘  └───┬─────┘
         │          │            │            │
         └──────────┴────────────┴────────────┘
                     │
...
|-------|---------|--------------|
| `Area` | Graph-based spatial model | - Adjacency map (Point → neighbors)<br>- Pathfinding support<br>- Nested `Point` class (x, y coordinates) |
| `Storage` | Warehouse container | - Maps Points → StorageCells<br>- 7×5 grid = 35 cells<br>- Charging station management<br>- Find/occupy/release operations |
| `StorageCell` | Storage unit | - 7 Types: AMBIENT, REFRIGERATED, BULK, CHARGING_STATION, LOADING, CORRIDOR, ANY<br>- Dimensions (width × depth × height)<br>- Capacity tracking<br>- Contains `BeveragesBox` collection |
...
**Warehouse Layout (7×5 Grid):**
```
     0    1    2    3    4    5    6
   ┌────┬────┬────┬────┬────┬────┬────┐
 0 │ A  │ A  │ =  │ R  │ R  │ =  │ B  │  Legend:
   │1A  │2A  │3A  │4A  │5A  │6A  │7A  │  A = Ambient
   ├────┼────┼────┼────┼────┼────┼────┤  R = Refrigerated
 1 │ A  │ A  │ =  │ R  │ R  │ =  │ B  │  B = Bulk
   │1B  │2B  │3B  │4B  │5B  │6B  │7B  │  C = Charging
   ├────┼────┼────┼────┼────┼────┼────┤  L = Loading
 2 │ A  │ A  │ =  │ =  │ =  │ =  │ =  │  = = Corridor
...
### **🗺️ Area Class & Graph-Based Spatial Model**
...
- ✅ Flexible layout definitions (not limited to grids)
- ✅ Obstacle representation (missing edges = blocked paths)
- ✅ Efficient pathfinding with Dijkstra's algorithm
- ✅ Reusable for both warehouse (7×5) and city (10×10) areas
...
- Zero-based coordinates: (0,0) = top-left, (6,4) = bottom-right (7×5 grid)
...
```
Point(0,0) → {Point(1,0), Point(0,1)}           // Top-left corner
Point(3,2) → {Point(2,2), Point(4,2), Point(3,1), Point(3,3)}  // Center node
Point(6,4) → {Point(5,4), Point(6,3)}           // Bottom-right corner
```
...
warehouseArea.setGraph(createGrid(7, 5));  // 7×5 = 35 nodes, ~70 edges
warehouseArea.setStart(0, 0);              // Default starting point
```

**City Area (10×10 Grid):**
```java
...
Point(0,0) → Point(1,0): √[(1-0)² + (0-0)²] = 1.0
Point(0,0) → Point(0,1): √[(0-0)² + (1-0)²] = 1.0
Point(0,0) → Point(1,1): √[(1-0)² + (1-0)²] = 1.414 (diagonal)
```
...
**Example 1: Warehouse Navigation (1A → 6D)**
...
Path: 1A → 2A → 3A → 3B → 3C → 4C → 5C → 6C → 7C → 7D
...
```java
// Truck traveling across city (10×10 grid)
Area cityArea = new Area();
...
Statement<?>[] program = {
    ...
    // Path 2: (0,0) → (1,0) → (2,0) → (3,0) [3 steps]
    ...
    // Path 3: (3,0) → (4,0) → (5,0) → (5,1) → (5,2) → (5,3) [5 steps]
...
// Battery drain: 8 × 5% = 40% battery used
```
...
Point(1,1) → {Point(0,1), Point(2,1)}  // No connection to (1,0) - obstacle!
...
- ⭐ One-way corridors (directed edges)
- ⭐ Variable edge weights (slow zones)
- ⭐ Multi-level warehouses (3D graphs)
...
**Charging Process:**
1. AGV detects low battery (≤20%)
2. Joins `CHARGING_QUEUE` (FIFO)
3. When station available → moves to it
4. Charges at 10%/tick
5. At 100% → releases station, returns to IDLE
...
| `PUSH value` | `[] → [value]` | Pushes value to stack | Store cell label |
| `MOVE` | `[label] → []` | Pops label, adds destination to `endPoints` queue | Navigate to "1A" |
| `TAKE` | `[box, label] → []` | Pops box & label, queues pickup operation | Take box from "1A" |
| `RELEASE` | `[box, label] → []` | Pops box & label, queues dropoff operation | Release box to "6D" |
| `CHARGE` | `[label] → []` | Pops label, queues charging operation | Charge at "1D" |
| `STOP` | `[...] → []` | Clears stack, `endPoints`, `operationsForEndPoints` | Emergency stop |
...
1. executeProgram() - Compiles program into queues
   ├─> Processes each Statement sequentially
   ├─> PUSH operations modify stack
   ├─> MOVE/TAKE/RELEASE operations:
   │   ├─> Pop arguments from stack
   │   ├─> Add destination to endPoints queue
   │   └─> Add operation closure to operationsForEndPoints
   └─> SETUP/STOP modify AGV state

2. onTick() - Executed every simulation tick
   ├─> Process endPoints queue (FIFO)
   ├─> Calculate path to next destination
   ├─> Move along path (1 step per tick, configurable)
   ├─> When destination reached:
   │   └─> Execute queued BeveragesBoxOperation
   └─> Repeat for next destination
```
...
|-------|---------|------------|---------|------------------|
| `MainFrame` | `JFrame` | - | Root window (1200×800px)<br>- Menu bar<br>- Panel coordination | On-demand |
| `WarehousePanel` | `JPanel` | `ITickable` | 7×5 storage grid<br>- Color-coded cells<br>- Charging stations | Every 5 ticks |
...
| `AGVComponent` | `JPanel` | `ITickable` | AGV visualization (80×80px)<br>- Animated wheels<br>- Battery indicator<br>- State display | Every tick |
| `TruckComponent` | `JPanel` | `ITickable` | Truck visualization (140×70px)<br>- Cargo display<br>- Load progress | Every tick |
...
├─────────────┬──────────────────────────────────────────┤
│ Warehouse   │  Loading Bays (Tabbed - 65%)            │
│ Panel       │  [Bay 1] [Bay 2] [Bay 3]                │
│ (7×5 Grid)  │  ┌─────────────────────────────┐        │
│             │  │ AGVComponent │TruckComponent│        │
│             │  └─────────────────────────────┘        │
│ Charging    │  Order Management (35%)                 │
│ Stations    │  Customer: ___  Beverage: [▼]          │
│             │  [Create Order] [Bulk Create]          │
│             │  ┌───────────────────────────────┐      │
│             │  │ Task Table (ID|Status|...)    │      │
│             │  └───────────────────────────────┘      │
├─────────────┴──────────────────────────────────────────┤
│ [▶Start] [⏸Pause] [⏹Stop] [Speed:■■■■□□] │ System Log│
└────────────────────────────────────────────────────────┘
```
...
2. CoreConfiguration.INSTANCE.autowire()
   ├─> AGVManagementConfiguration.autowire()
   ├─> TaskManagementConfiguration.autowire()
   ├─> StorageManagementConfiguration.autowire()
   ├─> ObservabilityConfiguration.autowire()
   ├─> Create system instances:
   │   ├─> ClockingSimulation
   │   ├─> TaskManagement
   │   ├─> StorageManagement
   │   └─> Observation
   └─> Start all 4 system threads

3. Setup Domain Objects
   ├─> setupCity() → 10×10 grid Area
   ├─> setupWarehouse() → 7×5 grid Storage
   ├─> setupTrucks() → 3 Truck instances
   ├─> setupAGVFleet() → 3 AGV instances
   └─> populateWarehouse() → 10 beverage types

4. Initialize AGV Charging
   └─> CoreConfiguration.initializeAGVChargingSystem(warehouse)
       └─> AGV.initializeChargingSystem(chargingStationCount)

5. GUIConfiguration.INSTANCE.autowire()
   ├─> Create MainFrame with system references
   ├─> Inject domain objects into panels
   ├─> Register GUI components as tickables
   └─> mainFrame.setVisible(true)

6. Thread.currentThread().join()
   └─> Keep main thread alive (systems run indefinitely)
```
...
ClockingSimulation (tick N):
   ...
   ├─> Truck-2.onTick(N)
   ├─> Truck-3.onTick(N)
   ├─> WarehousePanel.onTick(N)      [if N % 5 == 0]
   ├─> LoadingBayPanel.onTick(N)     [if N % 3 == 0]
   ├─> OrderManagementPanel.onTick(N) [if N % 10 == 0]
   └─> ControlLogPanel.onTick(N)     [if N % 10 == 0]
```
...
```bash
mvn package
java -jar target/capstone_alpha-0.0.1-SNAPSHOT.jar
```
...
- **API JavaDocs:** Run `mvn javadoc:javadoc` → `target/site/apidocs/`
