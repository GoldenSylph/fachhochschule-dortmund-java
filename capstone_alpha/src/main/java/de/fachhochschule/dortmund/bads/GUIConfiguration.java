...
public enum GUIConfiguration implements IConfiguration {
	...
	public IConfiguration autowire() {
		...
		SwingUtilities.invokeLater(() -> {
			...
			// Configure OrderManagementPanel with warehouse inventory
			if (primaryWarehouse != null) {
				mainFrame.getOrderManagementPanel().setStorage(primaryWarehouse);
				LOGGER.info("OrderManagementPanel configured with warehouse inventory ({} beverage types)",
						primaryWarehouse.getAvailableBeverageTypes().size());
			}
