...
public enum CoreConfiguration implements IConfiguration {
	...
	/**
	 * Initialize the AGV Task Dispatcher with the AGV fleet and warehouse.
	...
	public void initializeAGVTaskDispatcher(List<AGV> agvFleet, Storage warehouse) {
		...
		agvTaskDispatcher = new AGVTaskDispatcher(agvFleet, warehouse);
		LOGGER.info("AGV Task Dispatcher initialized with {} AGVs", agvFleet.size());
	}

	/**
	 * Get the AGV Task Dispatcher for assigning tasks to AGVs.
	 *
	 * @return the AGV Task Dispatcher instance, or null if not initialized
	 */
	public AGVTaskDispatcher getAGVTaskDispatcher() {
		return agvTaskDispatcher;
	}
