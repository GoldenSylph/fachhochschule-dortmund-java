...
public class App implements Runnable {
	...
	private void setupAGVFleet() {
		...
		// Initialize AGV Task Dispatcher for automatic task assignment
		CoreConfiguration.INSTANCE.initializeAGVTaskDispatcher(agvFleet, warehouse);
		LOGGER.info("AGV Task Dispatcher initialized and ready for order processing");
	}
	...
	private void populateWarehouse() {
		...
		// Ambient beverages
		if (createBeverageBox(BeveragesBox.Type.AMBIENT, "Water", 40, 30, 25, 24)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.AMBIENT, "Coca Cola", 40, 30, 25, 24)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.AMBIENT, "Sprite", 40, 30, 25, 24)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.AMBIENT, "Energy Drink", 35, 28, 22, 20)) successCount++; else failCount++;

		// Refrigerated beverages
		if (createBeverageBox(BeveragesBox.Type.REFRIGERATED, "Milk", 35, 30, 28, 12)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.REFRIGERATED, "Orange Juice", 35, 30, 28, 12)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.REFRIGERATED, "Yogurt Drink", 30, 25, 20, 16)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.REFRIGERATED, "Apple Juice", 35, 30, 28, 12)) successCount++; else failCount++;

		// Bulk items
		if (createBeverageBox(BeveragesBox.Type.BULK, "Beer Keg", 60, 60, 90, 1)) successCount++; else failCount++;
		if (createBeverageBox(BeveragesBox.Type.BULK, "Soda Syrup", 55, 55, 85, 1)) successCount++; else failCount++;

		LOGGER.info("Warehouse populated: {} beverages stored successfully, {} failed", successCount, failCount);
	}

	private boolean createBeverageBox(BeveragesBox.Type type, String name, int length, int width, int height, int qty) {
		BeveragesBox box = CoreConfiguration.INSTANCE.newBeveragesBox(type, name, length, width, height, qty);
		return warehouse.addBeverageToStorage(box);
	}
